...
**Formatos soportados:** PDF · imágenes (OCR) · Word `.docx .doc .dotx .docm`
· OpenDocument `.odt` · RTF · texto plano.

- **Parte 1 — Archivos nuevos (10):** crea cada archivo en la ruta indicada y pega el bloque completo.
- **Parte 2 — Archivos existentes (6):** solo hay que insertar fragmentos; cada uno indica dónde.

Rama de referencia: `feature/approval-document-translations` · commits `bad6d4c` + `493ca3c`
...
4. `Tools/services/office-extraction-service.js`
5. `Approvals_functions/services/translation-pdf-service.js`
6. `Approvals_functions/services/translation-job-runner.js`
7. `Approvals_functions/controllers/approval_translations.js`
8. `public/js/approval-translations.js`
9. `public/css/approval-translations.css`
10. `Approvals_functions/TRANSLATIONS.md`

### Parte 2 — Ediciones a archivos existentes
11. `Approvals.js`
12. `APPROVALS/routes/approvals-routes.js`
13. `Approvals_functions/controllers/approval_functions.js`
14. `public/scripts.js`
15. `views/layout.pug`
16. `package.json`
...
/* ============================================================
   ...
   Formatos soportados:
     ...
     .doc  .dot               Word 97-2003 binario  — requiere `word-extractor`

   Nota sobre .doc: el formato binario de Word 97-2003 es un
   contenedor OLE2 con estructuras propietarias. Escribir un parser
   propio seria fragil, asi que se delega en `word-extractor`, que se
   carga de forma perezosa: si no esta instalado, el resto de formatos
   sigue funcionando y el error indica exactamente que hacer.
...
function xmlToText(xml) {
    ...
    text = text.replace(/<\/table:table-cell>/g, '\t');
...
/**
 * Texto de un .doc / .dot binario (Word 97-2003).
 * Depende de `word-extractor`, cargado de forma perezosa.
 */
export async function extractFromLegacyDocBuffer(buffer) {
    let WordExtractor;
    try {
        ...
        const error = new Error(
            'Los archivos .doc (Word 97-2003) necesitan el paquete `word-extractor` ' +
            '(npm i word-extractor). Como alternativa, guarda el documento como .docx.'
        );
    ...
    return normalizeText([body, footnotes].filter(Boolean).join('\n\n'));
}
...
export async function extractFromDocument(buffer, filename) {
    ...
    switch (kind) {
        case 'word':
            ...
            break;
        case 'word-legacy':
            text = await extractFromLegacyDocBuffer(buffer);
            break;
        case 'opendocument':
 ...
 * Cubre PDF, imagenes (via OCR) y documentos ofimaticos
 * (Word, OpenDocument, RTF y texto plano).
 */
    ...
    static async createTranslation(connection, req, res) {
        ...
        if (!isTranslatableFile(filename)) {
            return res.status(415).send({
                result: 0,
                error: 'Only PDF, image (PNG, JPG, WEBP), Word (.docx, .doc), '
                     + 'OpenDocument, RTF and plain text files can be translated',
            });
        }
...
(function () {
  ...
  /** Icono FontAwesome segun la extension, para el encabezado de los modales. */
  function fileIconFor(filename) {
    ...
    if (/\.(odt|ott)$/.test(name)) return 'fa-file-alt';
    if (/\.rtf$/.test(name)) return 'fa-file-alt';
    if (/\.(txt|md|markdown|csv|log)$/.test(name)) return 'fa-file-alt';
    if (/\.(png|jpe?g|webp)$/.test(name)) return 'fa-file-image';
    return 'fa-file';
  }
  ...
  function openTranslateModal(rowId, filename) {
    ...
    el('atrSourceFilename').innerHTML = fileLabel(filename);
    setFeedback('');
  ...
  function openTranslationsModal(rowId, filename) {
    ...
    el('atrListFilename').innerHTML = fileLabel(filename);
...
archivo original (UNC)
   ├─ PDF        → extraction-service.js        texto embebido, y OCR si es escaneo
   ├─ Imagen     → extraction-service.js        siempre OCR
   └─ Documento  → office-extraction-service.js lectura directa del formato
...
| Formato | Extensiones | Cómo se lee |
|---|---|---|
| PDF | `.pdf` | Texto embebido; OCR si es escaneo |
| Imagen | `.png .jpg .jpeg .webp` | OCR (Tesseract) |
| Word moderno | `.docx .docm .dotx .dotm` | Nativo (OOXML), sin dependencias |
| Word 97-2003 | `.doc .dot` | `word-extractor` (opcional) |
| OpenDocument | `.odt .ott` | Nativo, sin dependencias |
| Rich Text | `.rtf` | Nativo, sin dependencias |
| Texto plano | `.txt .md .csv .log` | Nativo |

Los OOXML y ODT son ZIPs de los que solo interesa un XML, así que
`office-extraction-service.js` implementa un lector ZIP mínimo sobre `zlib`
(nativo de Node) en vez de arrastrar una librería completa. El `.doc` binario
es un contenedor OLE2 propietario: ahí sí se delega en `word-extractor`, que se
carga de forma perezosa — si no está instalado, el resto de formatos sigue
funcionando y el mensaje de error dice exactamente qué hacer.
...
| Dominio | `Tools/services/office-extraction-service.js` | Texto de Word/ODT/RTF/TXT. Vive en `Tools/` porque no es específico de Approvals. |
...
Agrega `@pdf-lib/fontkit` (fuentes Unicode) y, como dependencia **opcional**,
`word-extractor`, necesario solo para los `.doc` de Word 97-2003. Si tu `npm
install` corre con `--no-optional`, los `.doc` mostrarán un error explicativo y
el resto de formatos seguirá funcionando:

```bash
npm i word-extractor   # solo si necesitas soportar .doc antiguos
```
...
- **La salida siempre es PDF**, incluso si la entrada era un `.docx`. Traducir
  Word a Word conservando estilos es otro problema (y otra librería); el PDF
  cubre el caso de uso real, que es leer y compartir el contenido traducido.
...
**d)** En la rama **genérica** del final (`} else { // Other files: download`), después de `actions.appendChild(dlBtn);` — es la que habilita Word, ODT, RTF y TXT:

```javascript
                        // Word, OpenDocument, RTF y texto plano tambien se traducen.
                        appendTranslationActions(actions, id, item);
```
...
Y como bloque nuevo, justo antes de `"dependencies"` (solo si necesitas `.doc` de Word 97-2003):
...
# 3. Solo si necesitas .doc antiguos (Word 97-2003)
npm i word-extractor

# 4. Fuentes Unicode (solo para idiomas no latinos)
...
`.docx`, `.odt`, `.rtf` y `.txt` funcionan sin instalar nada extra.
