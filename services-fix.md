...
**Causa raíz:** `translateText()` hace `fetch` sin timeout. Si el endpoint de IA no
...
1. En el log de arranque debe aparecer:
   `[Translations] job runner started (concurrency 2, job timeout 15m, sources: crm, approvals)`
   Si `crm` no está en la lista, el import por efecto secundario de
   `Approvals.js:29` no se ejecutó.
